...
document.getElementById('send-first-diagnosis').addEventListener('click', () => {
    ...
    if (!authorization) {
        return alert('Se necesita la autorización del usuario.')
    }
    ...
    if (first_diagnosis === '') {
        return alert('Ingresar un diagnóstico.')
    }
...
document.getElementById('send-tech').addEventListener('click', () => {
    ...
    const listofRepairs = JSON.parse(localStorage.getItem('repairs')) || [];
    ...
    alert('Reparación creada y asignada correctamente');
