...
const UserModel = {
  async findOrCreateFromGoogle(profile) {
    ...
    const username = profile.emails?.[0]?.value?.split('@')[0] || profile.displayName || `user_${Date.now()}`;
...
const PackageModel = {
  ...
  async generateTracking() {
    ...
    let tries = 0;
    while (tries < 25) {
      tracking = 'PKG' + String(Math.floor(Math.random() * 1e10)).padStart(10, '0');
      const [r] = await pool.execute('SELECT COUNT(*) AS c FROM packages WHERE tracking_number = ?', [tracking]);
      if (r[0].c === 0) break;
      tries++;
    }
    return tracking;
