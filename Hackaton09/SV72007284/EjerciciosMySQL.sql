...
order by total_rentas desc limit 10;
...
select a.first_name, a.last_name, count(fa.film_id) as total_peliculas from actor a
    ...
    group by a.actor_id
    order by total_peliculas desc limit 10;
...
select c.first_name, c.last_name, sum(p.amount) as total from customer c
    ...
    having total > (
        ...
        from (select sum(amount) total_cliente 
        from payment group by customer_id) t);
...
group by day
order by total_day desc limit 1;
...
select city, total from (
	select city, total, row_number() over (order by total desc) as rn
    from ingresos_ciudad) t
where rn <= 3;

# Usar un CTE para obtener los empleados que registraron más rentas que el promedio de todos.
WITH rentas_empleado AS (
    SELECT 
        s.staff_id,
        s.first_name,
        s.last_name,
...
promedio AS (
    ...
    FROM rentas_empleado
)
SELECT re.staff_id, re.first_name, re.last_name, re.total
FROM rentas_empleado re
JOIN promedio p ON re.total > p.avg_total;
...
SELECT 
    title,
    total_rentas,
    RANK() OVER (ORDER BY total_rentas DESC) AS ranking
...
SELECT 
    ...
    title,
    total_rentas,
    RANK() OVER (PARTITION BY categoria ORDER BY total_rentas DESC) AS ranking
FROM peliculas_rentas;
...
SELECT 
    ...
    last_name,
    total_rentas,
    total_rentas - AVG(total_rentas) OVER () AS diferencia_vs_promedio
FROM rentas_cliente;
...
SELECT * FROM (
    SELECT 
        ...
        last_name,
        total_peliculas,
        ROW_NUMBER() OVER (ORDER BY total_peliculas DESC) AS rn
    FROM actores_peliculas) t
WHERE rn <= 5;

# 10. Retos Finales
# Encontrar la película con el mayor número de actores distintos.
SELECT f.title, COUNT(DISTINCT fa.actor_id) AS total_actores
FROM film f
    JOIN film_actor fa ON f.film_id = fa.film_id
GROUP BY f.film_id, f.title
...
WITH ingresos_pelicula AS (
    SELECT f.film_id, f.title, SUM(p.amount) AS total_ingreso
    FROM film f
        JOIN inventory i ON f.film_id = i.film_id
        JOIN rental r ON i.inventory_id = r.inventory_id
...
promedio AS (
    SELECT AVG(total_ingreso) AS promedio_general
    FROM ingresos_pelicula
)
SELECT i.film_id, i.title, i.total_ingreso
FROM ingresos_pelicula i
JOIN promedio p ON i.total_ingreso > p.promedio_general
ORDER BY i.total_ingreso DESC;
...
peliculas_cliente AS (
    SELECT 
        ...
        tc.total,
        f.title, 
        COUNT(r.rental_id) AS veces
    FROM top_clientes tc
        JOIN rental r ON tc.customer_id = r.customer_id
        JOIN inventory i ON r.inventory_id = i.inventory_id
        JOIN film f ON i.film_id = f.film_id
    GROUP BY tc.customer_id, tc.first_name, tc.last_name, tc.total, f.film_id, f.title
)
SELECT * FROM (
    SELECT 
        pc.*,
        ROW_NUMBER() OVER (PARTITION BY pc.customer_id ORDER BY veces DESC) AS rn
    FROM peliculas_cliente pc
) t
WHERE rn = 1
